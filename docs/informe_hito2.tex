...
J & Experimento & La consulta de materiales populares agrega cada tabla de
    interacción en subconsultas independientes para no multiplicar filas
    (evita el producto cartesiano de unir tres relaciones 1-a-muchos). \\
K & Experimento & Los índices del experimento se ubican sobre columnas que no
    lideran ninguna PK: son las que producen un contraste real entre el
    escenario con y sin índices. \\
...
resultados del experimento. El cambio J describe la forma de la consulta de
popularidad: en lugar de unir las tres tablas de interacción con
\texttt{LEFT JOIN} directos sobre \texttt{Material} ---lo que generaría un
producto cartesiano (filas de likes por filas de lecturas por filas de reseñas
por cada material)---, la consulta agrega cada tabla por separado en
subconsultas independientes y luego las combina, de modo que el tiempo que se
mide refleja el costo real de una agregación total y no un error de
formulación. El cambio K explica dónde se colocan los índices del experimento:
sobre columnas que no son la primera de ninguna PK. Una columna que lidera una
PK ---como \texttt{Material\_Id} en \texttt{Escribe}--- ya tiene un índice de PK
encima, así que un índice secundario adicional no cambiaría nada; los índices
del benchmark se ubican justo en las columnas donde sí aparece contraste entre
el escenario con y sin índices.
...
(\texttt{BETWEEN}) ---Q1 sobre el código de clasificación de edad y Q4 sobre
el año de publicación---, que es el caso más ilustrativo del funcionamiento de
un índice B-tree.

\begin{center}
\begin{longtable}{p{0.9cm}p{4.6cm}p{4.2cm}p{3.0cm}}
\toprule
...
Q1 & Materiales de un género acotados por clasificación de edad
  & Igualdad (género) + rango (\texttt{code BETWEEN})
  & \texttt{idx\_pertenece\_genero}, \texttt{idx\_material\_agerate} \\
Q2 & Los 20 materiales más populares (ranking)
  & Agregación total (subconsultas independientes)
  & \texttt{idx\_resena\_material} \\
Q3 & Historial de lectura de un usuario
  & Igualdad selectiva + filtro temporal + orden
  & \texttt{idx\_leer\_usuario} \\
Q4 & Usuarios con reseñas destacadas por rango de años
  & \textbf{Rango} (\texttt{año BETWEEN}) + puntaje alto + \texttt{DISTINCT}
...
\paragraph{Q4 --- Usuarios con reseñas destacadas por rango de años.}
...
El predicado de \emph{rango} sobre \texttt{Anio\_Publicacion}
(\texttt{BETWEEN 2000 AND 2005}) es el caso más ilustrativo de un índice
B-tree: el índice \texttt{idx\_material\_anio} permite recorrer solo el
tramo de años pedido en lugar de leer la tabla completa. El filtro adicional
por puntaje alto (\texttt{Puntaje > 8.5}) se apoya en
\texttt{idx\_resena\_material\_punt}, y el \texttt{DISTINCT} colapsa los
usuarios que califican con más de una reseña.

\paragraph{Q3 --- Historial de lectura de un usuario.}
...
\begin{itemize}
  ...
  de ninguna PK (\texttt{Genero\_Nombre}, \texttt{agerate\_code},
  \texttt{Usuario\_Username}/\texttt{Email}, \texttt{Anio\_Publicacion},
  \texttt{Material\_Id} en \texttt{Resena})
  o son agregaciones totales donde ningún índice aplica (Q2).
...
El experimento usa diez índices B-tree no únicos. Todos se ubican sobre
columnas que no lideran ninguna PK (Sección~\ref{sec:cambios}, cambio K):

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Nombre} & \textbf{Definición} & \textbf{Apoya} \\
\midrule
\texttt{idx\_pertenece\_genero}   & \texttt{Pertenece(Genero\_Nombre)} & Q1 \\
\texttt{idx\_material\_agerate}   & \texttt{Material(agerate\_code)} & Q1 \\
\texttt{idx\_agerate\_code}       & \texttt{AgeRate(code)} & Q1 \\
\texttt{idx\_resena\_material}    & \texttt{Resena(Material\_Id)} & Q2, Q4 \\
\texttt{idx\_resena\_material\_punt} & \texttt{Resena(Material\_Id, Puntaje)} & Q2, Q4 \\
\texttt{idx\_likes\_material}     & \texttt{Likes(Material\_Id)} & Q2 \\
\texttt{idx\_leer\_material}      & \texttt{Leer(Material\_Id)} & Q2 \\
\texttt{idx\_leer\_usuario}       & \texttt{Leer(Usuario\_Username, Usuario\_Email, Fecha DESC)} & Q3 \\
\texttt{idx\_material\_anio}      & \texttt{Material(Anio\_Publicacion)} & Q4 \\
\texttt{idx\_escribe\_autor}      & \texttt{Escribe(Autor\_Id)} & apoyo \\
\bottomrule
\end{tabular}
\end{center}

La PK de \texttt{Resena} es \texttt{Code}, de modo que \texttt{Material\_Id}
no queda cubierto por ningún índice de PK: por eso \texttt{idx\_resena\_material}
y \texttt{idx\_resena\_material\_punt} sí producen contraste en Q2 y Q4.
...
%      \i sql/06_experiments.sql
%
%   Con esos 32 valores (4 consultas × 4 bases × 2 escenarios) se completan
...
Q4 (rango años)   &  890 &  915 & 1139 & 3831 \\
\bottomrule
\end{tabular}
\end{center}
...
Q4 (rango años)   &  817 &  913 & 1070 & 3038 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Lectura de las tablas.} En 1K y 10K el índice apenas se nota: en
tablas pequeñas el planificador a menudo ignora el índice y hace
\texttt{Seq Scan}, y dos casos (Q2 y Q3 en 1K) incluso salen levemente más
lentos \emph{con} índice (881 vs.\ 848 ms en Q2; 297 vs.\ 293 ms en Q3). No es
un error: es ruido de medición sobre tiempos casi idénticos y evidencia de que
un índice no siempre ayuda. El contraste real aparece en 100K y 1M, cuando la
tabla ya no cabe holgada en memoria y el plan basado en índice gana terreno
(\emph{crossover} por selectividad). El mayor beneficio relativo es Q3, que en
1M baja de 555 a 261 ms ($-53\%$); Q2, una agregación total, es el que menos
mejora.
...
\subsubsection{Q1 --- Materiales de un género acotados por clasificación de edad}

\textit{[Pendiente. Incluir: tiempos, plan EXPLAIN sin y con índice,
explicación de cómo el filtro por igualdad de género (\texttt{idx\_pertenece\_genero})
y el rango sobre \texttt{code} (\texttt{idx\_material\_agerate},
\texttt{idx\_agerate\_code}) combinan acceso por igualdad y por rango.]}

\subsubsection{Q2 --- Materiales más populares}

\textit{[Pendiente. Incluir: tiempos, plan EXPLAIN sin y con índice,
explicación de por qué el índice no aporta en agregaciones totales (en 1M baja
solo de 7432 a 5557 ms) y por qué \texttt{idx\_resena\_material} es el único
que ayuda, dado que \texttt{Material\_Id} no lidera la PK de \texttt{Resena}.]}

\subsubsection{Q3 --- Historial de lectura de un usuario}

\textit{[Pendiente. Incluir: tiempos, plan EXPLAIN sin y con índice,
explicación de \texttt{idx\_leer\_usuario} y cómo, además de filtrar por
usuario y por el año de la lectura, entrega el orden ya resuelto y evita el
paso de Sort. Es el mayor ganador del índice: en 1M baja de 555 a 261 ms.]}

\subsubsection{Q4 --- Usuarios con reseñas destacadas por rango de años}

\textit{[Pendiente. Incluir: tiempos, plan EXPLAIN sin y con índice,
explicación del Bitmap Index Scan sobre \texttt{idx\_material\_anio} y la
bondad del índice B-tree en predicados BETWEEN selectivos, junto con el filtro
por puntaje alto y el \texttt{DISTINCT}. También analizar qué ocurre con rangos
...
\begin{itemize}
  ...
  usuario. Los doce ajustes al modelo se documentaron de forma transparente.
  ...
  del índice y en qué medida, el comportamiento de la consulta por rango (Q4),
...
\subsection{Planes de ejecución (Q4, base 1M)}

\textit{[Pendiente. Capturar la salida de \texttt{EXPLAIN (ANALYZE, BUFFERS)}
para la consulta Q4 sobre \texttt{bd\_literaria\_1m}, primero sin el índice
